...
def evaluate_generations(generations : dict[str, list], mode):
    # Load the samples
    dataset = [json.loads(l) for l in open(os.path.join(BASE_DIR, "data/cruxeval.jsonl"), "r").readlines()]
    references = [(doc["code"], doc["input"], doc["output"]) for doc in dataset]

    new_generations = {}
    for k, v in generations.items():
        new_generations[f"sample_{int(k)}"] = v
    generations = new_generations

    total_num = len(generations)
    print(f"Evaluating on {total_num} samples...")

    generations_list = [generations[f"sample_{i}"] for i in range(total_num)]
